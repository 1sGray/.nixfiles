...
sudo nix-shell -p git --command "git clone https://github.com/1sGray/.nixfiles$SCRIPT_DIR"
...
#sed -i "0,/gray/s//$(whoami)/" $SCRIPT_DIR/flake.nix
#sed -i "0,/gray/s//$(getent passwd $(whoami) | cut -d ':' -f 5 | cut -d ',' -f 1)/" $SCRIPT_DIR/flake.nix
#sed -i "s/1s_gray@proton.me//" $SCRIPT_DIR/flake.nix
#sed -i "s+~/.nixtfiles+$SCRIPT_DIR+g" $SCRIPT_DIR/flake.nix
